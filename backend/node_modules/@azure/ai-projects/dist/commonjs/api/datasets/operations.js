...
exports._getCredentialsSend = _getCredentialsSend;
exports._getCredentialsDeserialize = _getCredentialsDeserialize;
exports.getCredentials = getCredentials;
exports._pendingUploadSend = _pendingUploadSend;
exports._pendingUploadDeserialize = _pendingUploadDeserialize;
exports.pendingUpload = pendingUpload;
...
function _getCredentialsSend(context, name, version, options = { requestOptions: {} }) {
    const path = (0, urlTemplate_js_1.expandUrlTemplate)("/datasets/{name}/versions/{version}/credentials{?api-version}", {
        name: name,
        version: version,
...
async function _getCredentialsDeserialize(result) {
    ...
    return (0, models_js_1.assetCredentialResponseDeserializer)(result.body);
}
...
function _pendingUploadSend(context, name, version, pendingUploadRequest, options = { requestOptions: {} }) {
    const path = (0, urlTemplate_js_1.expandUrlTemplate)("/datasets/{name}/versions/{version}/startPendingUpload{?api-version}", {
        name: name,
        version: version,
        "api-version": context.apiVersion,
    }, {
    ...
    return context.path(path).post({
        ...
        body: (0, models_js_1.pendingUploadRequestSerializer)(pendingUploadRequest),
    });
}
async function _pendingUploadDeserialize(result) {
    ...
    return (0, models_js_1.pendingUploadResponseDeserializer)(result.body);
}
...
async function createDatasetAndGetItsContainer(context, name, version, options) {
    ...
    const containerClient = new storage_blob_1.ContainerClient(blobReference.credential.sasUri);
...
async function uploadFile(context, name, version, filePath, options) {
    ...
    const isDirectory = fs_1.default.lstatSync(filePath).isDirectory();
    if (isDirectory) {
    ...
    await blockBlobClient.uploadStream(fs_1.default.createReadStream(filePath));
...
async function uploadFolder(context, name, version, folderPath, options) {
    ...
    const folderExists = fs_1.default.existsSync(folderPath);
    if (!folderExists) {
        throw new Error(`Folder does not exist at path: ${folderPath}`);
    }
    // Check if the folder is a file
    const isFile = fs_1.default.lstatSync(folderPath).isFile();
    if (isFile) {
    ...
    for (const filePath of filteredFiles) {
        ...
        const relativePath = path_1.default.relative(folderPath, filePath).split(path_1.default.sep).join("/");
...
function _createOrUpdateSend(context, name, version, datasetVersion, options = { requestOptions: {} }) {
    const path = (0, urlTemplate_js_1.expandUrlTemplate)("/datasets/{name}/versions/{version}{?api-version}", {
        name: name,
        version: version,
        "api-version": context.apiVersion,
    }, {
    ...
    return context.path(path).patch({
        ...
        body: (0, models_js_1.datasetVersionUnionSerializer)(datasetVersion),
    });
}
async function _createOrUpdateDeserialize(result) {
    ...
    return (0, models_js_1.datasetVersionUnionDeserializer)(result.body);
}
...
function _listSend(context, options = { requestOptions: {} }) {
    const path = (0, urlTemplate_js_1.expandUrlTemplate)("/datasets{?api-version}", {
        "api-version": context.apiVersion,
    }, {
        allowReserved: options?.requestOptions?.skipUrlEncoding,
    });
...
async function _listDeserialize(result) {
    ...
    return (0, models_js_1._pagedDatasetVersionDeserializer)(result.body);
}
...
function _listVersionsSend(context, name, options = { requestOptions: {} }) {
    const path = (0, urlTemplate_js_1.expandUrlTemplate)("/datasets/{name}/versions{?api-version}", {
        name: name,
        "api-version": context.apiVersion,
    }, {
        allowReserved: options?.requestOptions?.skipUrlEncoding,
